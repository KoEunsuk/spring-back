...
public class DriverService {
    ...
    public DriverDto updateDriver(Long id, DriverDto updatedDriver) {
        driverRepository.findById(id) // 해당 ID의 운전자가 있는지 확인
                .orElseThrow(() -> new RuntimeException("운전자를 찾을 수 없습니다: " + id));
        updatedDriver.setId(id); // URL 경로의 ID를 DTO에 설정 (업데이트할 대상 ID 명확화)
        return driverRepository.save(updatedDriver); // 업데이트 수행
    }
