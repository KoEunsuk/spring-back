...
public class BusService {
    ...
    private BusDto toDto(Bus entity) {
        return new BusDto(
                ...
                entity.getOperator() != null ? entity.getOperator().getOperatorName() : null
        );
    }
    ...
    public BusDto updateBus(Long id, BusDto busDto) {
        ...
        existingBus.setRouteNumber(busDto.getRouteNumber());
        existingBus.setRouteType(busDto.getRouteType());
        existingBus.setCapacity(busDto.getCapacity());

        if (!existingBus.getVehicleNumber().equals(busDto.getVehicleNumber())) {
            if (busRepository.findByVehicleNumber(busDto.getVehicleNumber()).isPresent()) {
                throw new IllegalArgumentException("이미 존재하는 차량 번호입니다.");
            }
            existingBus.setVehicleNumber(busDto.getVehicleNumber());
        }

        existingBus.setVehicleType(busDto.getVehicleType());
        existingBus.setVehicleYear(busDto.getVehicleYear());
        existingBus.setLastMaintenance(busDto.getLastMaintenance());
        existingBus.setRepairCount(busDto.getRepairCount());

        if (busDto.getOperatorId() != null) {
            Operator operator = operatorRepository.findById(busDto.getOperatorId())
                    .orElseThrow(() -> new ResourceNotFoundException("Operator", "id", busDto.getOperatorId()));
