...
public class AdminDispatchController {
    ...
    // 신규 배차 생성
    @PostMapping
    public ResponseEntity<ApiResponse<DispatchDetailResponse>> createDispatch(
            @Valid @RequestBody DispatchCreateRequest createRequest,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        DispatchDetailResponse responseData = dispatchService.createDispatch(createRequest, userDetails);
    ...
    public ResponseEntity<ApiResponse<List<DispatchDetailResponse>>> getAllDispatches(
        ...
        List<DispatchDetailResponse> dispatches = dispatchService.getDispatchesForAdmin(startDate, endDate, statuses, userDetails);
        return ResponseEntity.ok(ApiResponse.success("배차 목록 조회에 성공했습니다.", dispatches));
    }

    // 특정 배차 상세 조회
    @GetMapping("/{dispatchId}")
    public ResponseEntity<ApiResponse<DispatchDetailResponse>> getDispatchById(
    ...
    public ResponseEntity<ApiResponse<List<BusDetailResponse>>> getAvailableBuses(
        ...
        List<BusDetailResponse> availableBuses = dispatchService.findAvailableBuses(startTime, endTime, userDetails);
    ...
    public ResponseEntity<ApiResponse<List<DriverDetailResponse>>> getAvailableDrivers(
        ...
        List<DriverDetailResponse> availableDrivers = dispatchService.findAvailableDrivers(startTime, endTime, userDetails);
    ...
    public ResponseEntity<ApiResponse<DispatchDetailResponse>> cancelDispatch(@PathVariable Long dispatchId,
                                                                              @AuthenticationPrincipal CustomUserDetails userDetails) {
        DispatchDetailResponse responseData = dispatchService.cancelDispatch(dispatchId, userDetails);
    ...
    public ResponseEntity<ApiResponse<List<LocationHistoryResponse>>> getLocationHistories(@PathVariable Long dispatchId,
                                                                                           @AuthenticationPrincipal CustomUserDetails userDetails) {
        List<LocationHistoryResponse> responseData = dispatchService.getLocationHistoriesForDispatch(dispatchId, userDetails);
