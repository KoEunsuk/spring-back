...
## ## 인증 시스템 고도화: 토큰 관리 리팩토링 트러블슈팅

### ### 1. 문제점: 단일 토큰 사용으로 인한 사용자 불편 및 보안 취약성

* **📌 현상**:
    * 기존 시스템은 유효기간이 24시간인 단일 JWT(Access Token)만을 사용했습니다.
    * 이로 인해 사용자는 매일 로그인을 다시 해야 하는 불편함을 겪었습니다.
    * 만약 Access Token이 탈취될 경우, 24시간이라는 긴 시간 동안 공격에 노출되는 보안적 위험이 있었습니다.

* **💡 해결 과정**:
    * 인증 시스템을 **Access Token**과 **Refresh Token**으로 이원화했습니다.
        * **Access Token**: 유효기간을 **1시간**으로 짧게 설정하고, 실제 API 요청 시 권한 증명에만 사용하도록 역할을 제한했습니다.
        * **Refresh Token**: 유효기간을 **7일**로 길게 설정하고, 만료된 Access Token을 재발급받는 용도로만 사용하도록 했습니다.
    * `/api/auth/refresh` 엔드포인트를 구현하여, Access Token 만료 시 클라이언트가 Refresh Token을 통해 새로운 Access Token을 자동으로 발급받는 흐름을 완성했습니다.

* **✅ 결과**:
    * 사용자는 Refresh Token 덕분에 로그인 상태를 오랫동안 유지할 수 있게 되어 **사용자 경험(UX)이 크게 향상**되었습니다.
    * Access Token의 유효기간이 짧아져 토큰 탈취 시 **보안 위험이 감소**했습니다.

### ### 2. 문제점: Refresh Token의 안전하지 않은 저장 방식

* **📌 현상**:
    * 유효기간이 긴 Refresh Token을 클라이언트의 `localStorage` 등에 저장할 경우, XSS(Cross-Site Scripting) 공격에 의해 토큰이 탈취될 위험이 존재했습니다.
    * 또한, 웹과 앱 클라이언트를 모두 지원해야 하는 요구사항이 있었습니다.

* **💡 해결 과정**:
    * **웹 클라이언트**를 위해, 서버는 Refresh Token을 **`HttpOnly` 보안 쿠키**에 담아 전송하도록 구현했습니다. `HttpOnly` 속성은 JavaScript가 쿠키에 접근하는 것을 막아 XSS 공격을 원천적으로 차단합니다.
    * **앱 클라이언트**는 쿠키를 다루기 어려우므로, 하위 호환성을 위해 **JSON 응답 본문(Body)**에도 Refresh Token을 함께 담아 보내도록 설계했습니다. 앱은 이 값을 받아 자체 보안 스토리지(예: Keychain, Keystore)에 저장합니다.

* **✅ 결과**:
    * 단일 API 엔드포인트로 **웹과 앱 클라이언트를 모두 지원**하면서, 웹 환경에서의 **보안성을 크게 강화**했습니다.

### ### 3. 문제점: 서버에서 세션을 무효화할 수 없는 Stateless 한계

* **📌 현상**:
    * JWT의 특징인 Stateless(무상태) 때문에, 클라이언트에서 토큰을 삭제하여 로그아웃하더라도 서버는 해당 토큰이 만료되기 전까지 유효하다고 판단합니다.
    * 이는 관리자가 특정 사용자를 강제로 로그아웃시키거나, 사용자가 다른 기기에서 로그인했을 때 기존 로그인을 무효화하는 등의 기능을 구현할 수 없음을 의미했습니다.

* **💡 해결 과정**:
    * `RefreshToken` 엔티티와 리포지토리를 생성하여, 발급된 모든 유효한 Refresh Token을 **데이터베이스에 저장**하도록 변경했습니다.
    * `/api/auth/logout` 엔드포인트를 구현하여, 로그아웃 요청 시 DB에서 해당 Refresh Token을 **물리적으로 삭제**하도록 했습니다.
    * (추가) 보안 강화를 위해, DB에 저장하는 Refresh Token은 **해싱(`PasswordEncoder`)**하여 저장하도록 구현했습니다.

* **✅ 결과**:
    * Stateless한 JWT 인증 방식에 **Stateful한 세션 관리**의 장점을 결합했습니다.
    * 이제 서버가 명시적으로 토큰을 무효화할 수 있게 되어 **강제 로그아웃 등 완전한 세션 제어**가 가능해졌습니다.
